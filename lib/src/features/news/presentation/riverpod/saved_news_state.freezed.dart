// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'saved_news_state.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#adding-getters-and-methods-to-our-models');

/// @nodoc
mixin _$SavedNewsState {
  LoadState get loadState => throw _privateConstructorUsedError;
  List<ArticleEntity> get savedArticles => throw _privateConstructorUsedError;

  @JsonKey(ignore: true)
  $SavedNewsStateCopyWith<SavedNewsState> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $SavedNewsStateCopyWith<$Res> {
  factory $SavedNewsStateCopyWith(
          SavedNewsState value, $Res Function(SavedNewsState) then) =
      _$SavedNewsStateCopyWithImpl<$Res, SavedNewsState>;
  @useResult
  $Res call({LoadState loadState, List<ArticleEntity> savedArticles});
}

/// @nodoc
class _$SavedNewsStateCopyWithImpl<$Res, $Val extends SavedNewsState>
    implements $SavedNewsStateCopyWith<$Res> {
  _$SavedNewsStateCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? loadState = null,
    Object? savedArticles = null,
  }) {
    return _then(_value.copyWith(
      loadState: null == loadState
          ? _value.loadState
          : loadState // ignore: cast_nullable_to_non_nullable
              as LoadState,
      savedArticles: null == savedArticles
          ? _value.savedArticles
          : savedArticles // ignore: cast_nullable_to_non_nullable
              as List<ArticleEntity>,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$SavedNewsStateImplCopyWith<$Res>
    implements $SavedNewsStateCopyWith<$Res> {
  factory _$$SavedNewsStateImplCopyWith(_$SavedNewsStateImpl value,
          $Res Function(_$SavedNewsStateImpl) then) =
      __$$SavedNewsStateImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call({LoadState loadState, List<ArticleEntity> savedArticles});
}

/// @nodoc
class __$$SavedNewsStateImplCopyWithImpl<$Res>
    extends _$SavedNewsStateCopyWithImpl<$Res, _$SavedNewsStateImpl>
    implements _$$SavedNewsStateImplCopyWith<$Res> {
  __$$SavedNewsStateImplCopyWithImpl(
      _$SavedNewsStateImpl _value, $Res Function(_$SavedNewsStateImpl) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? loadState = null,
    Object? savedArticles = null,
  }) {
    return _then(_$SavedNewsStateImpl(
      loadState: null == loadState
          ? _value.loadState
          : loadState // ignore: cast_nullable_to_non_nullable
              as LoadState,
      savedArticles: null == savedArticles
          ? _value._savedArticles
          : savedArticles // ignore: cast_nullable_to_non_nullable
              as List<ArticleEntity>,
    ));
  }
}

/// @nodoc

class _$SavedNewsStateImpl implements _SavedNewsState {
  const _$SavedNewsStateImpl(
      {this.loadState = const Empty(),
      final List<ArticleEntity> savedArticles = const []})
      : _savedArticles = savedArticles;

  @override
  @JsonKey()
  final LoadState loadState;
  final List<ArticleEntity> _savedArticles;
  @override
  @JsonKey()
  List<ArticleEntity> get savedArticles {
    if (_savedArticles is EqualUnmodifiableListView) return _savedArticles;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_savedArticles);
  }

  @override
  String toString() {
    return 'SavedNewsState(loadState: $loadState, savedArticles: $savedArticles)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$SavedNewsStateImpl &&
            (identical(other.loadState, loadState) ||
                other.loadState == loadState) &&
            const DeepCollectionEquality()
                .equals(other._savedArticles, _savedArticles));
  }

  @override
  int get hashCode => Object.hash(runtimeType, loadState,
      const DeepCollectionEquality().hash(_savedArticles));

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$SavedNewsStateImplCopyWith<_$SavedNewsStateImpl> get copyWith =>
      __$$SavedNewsStateImplCopyWithImpl<_$SavedNewsStateImpl>(
          this, _$identity);
}

abstract class _SavedNewsState implements SavedNewsState {
  const factory _SavedNewsState(
      {final LoadState loadState,
      final List<ArticleEntity> savedArticles}) = _$SavedNewsStateImpl;

  @override
  LoadState get loadState;
  @override
  List<ArticleEntity> get savedArticles;
  @override
  @JsonKey(ignore: true)
  _$$SavedNewsStateImplCopyWith<_$SavedNewsStateImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
